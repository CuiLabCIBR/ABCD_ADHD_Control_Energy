...
counts <- track_exclusions(sub_info, counts, "Excessive head motion: FD > mean + 3×SD")
...
In this work, the cognition performance of ABCD participants was measured by the [NIH Toolbox Cognition Battery](https://nihtoolbox.org/domain/cognition/). The **NIH Toolbox Cognition Battery**, recommended for ages 7+, consists of tests of multiple constructs. It yields individual test scores and the following summary scores: **Total Cognition Composite, Fluid Composite** (includes Dimensional Change Card Sort, Flanker Inhibitory Control and Attention, Picture Sequence Memory (Form A), List Sorting Working Memory, and Pattern Comparison tests), and **Crystallized Composite** (includes Picture Vocabulary and Oral Reading Recognition tests).

::: callout-note
In this study, the uncorrected standard score was analyzed. The NIH Toolbox Scoring and Interpretation Guide can be found [here](https://www.epicrehab.com/epic/documents/crc/crc-201307-nih-toolbox-scoring-and-interpretation-manual%209-27-12.pdf).
:::

**Single Test Performance**

1.  Picture Vocabulary Test (nihtbx_picvocab_uncorrected)
2.  Flanker Inhibitory Control and Attention Test (nihtbx_flanker_uncorrected)
3.  List Sorting Working Memory Test (nihtbx_list_uncorrected)
4.  Dimensional Change Card Sort Test (nihtbx_cardsort_uncorrected)
5.  Pattern Comparison Processing Speed (nihtbx_pattern_uncorrected)
6.  Picture Sequence Memory Test (nihtbx_picture_uncorrected)
7.  Oral Reading Recognition Test (nihtbx_reading_uncorrected)

**Cognition Composite Score**

1.  Total Cognition Composite Score (nihtbx_totalcomp_uncorrected)
2.  Fluid Composite Score (nihtbx_fluidcomp_uncorrected)
3.  Crystallized Composite Score (nihtbx_cryst_uncorrected)

```{r cogcomp}
nc_y_nihtb <- fread(paste0(demo_path, "neurocognition/nc_y_nihtb.csv"))
nc_y_nihtb <- as.data.frame(nc_y_nihtb)

cog_item <- nc_y_nihtb %>% 
  select(ends_with("_uncorrected")) %>% 
  colnames()

sub_info <- merge(sub_info, nc_y_nihtb[, c("src_subject_id", "eventname", cog_item)], 
                  by = c("src_subject_id", "eventname"), all.x = TRUE)
```

**Emotional N-Back**

1.  The rate of correct responses to 0 back stimuli during run 1 and run 2 (tfmri_nb_all_beh_c0b_rate)
2.  The rate of correct responses to 2 back stimuli during run 1 and run 2 (tfmri_nb_all_beh_c2b_rate)
3.  The rate of all correct responses (tfmri_nb_all_beh_ctotal_rate)
4.  Average reaction time for all correct responses to 0 back stimuli during run 1 and run 2 (tfmri_nb_all_beh_c0b_mrt)
5.  Average reaction time for all correct responses to 2 back stimuli during run 1 and run 2 (tfmri_nb_all_beh_c2b_mrt)
6.  Mean reaction time for all correct responses (tfmri_nb_all_beh_ctotal_mrt)

```{r nBack}
mri_y_tfmr_nback_beh <- fread(paste0(demo_path, "imaging/mri_y_tfmr_nback_beh.csv"))
mri_y_tfmr_nback_beh <- as.data.frame(mri_y_tfmr_nback_beh)

nback_item <- c("tfmri_nb_all_beh_c0b_rate", "tfmri_nb_all_beh_c2b_rate", "tfmri_nb_all_beh_ctotal_rate",
                "tfmri_nb_all_beh_c0b_mrt", "tfmri_nb_all_beh_c2b_mrt", "tfmri_nb_all_beh_ctotal_mrt")

sub_info <- merge(sub_info, mri_y_tfmr_nback_beh[, c("src_subject_id", "eventname", nback_item)], 
                  by = c("src_subject_id", "eventname"), all.x = TRUE)
