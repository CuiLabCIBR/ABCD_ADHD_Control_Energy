---
title: "ADHD Energy Deviation Biotype - Nonlinear Age Effects Validation"
author:
  - name: Hang Yang
...
source("F:/Cui_Lab/Projects/ABCD_ADHD_Control_Energy/functions/calc_w_scores_gam.R")
...
outpath_method <- file.path(root_path, "results", task_contrast, atlas, "gam")
...
gam_data <- cbind(energy_wholebrain, behav_data)
W_scores_wholebrain <- calc_w_scores_gam(gam_data[train_idx, ], gam_data[test_idx, ], covariates)
...
gam_data <- cbind(energy_roi, behav_data)
W_scores_roi <- calc_w_scores_gam(gam_data[train_idx, ], gam_data[test_idx, ], covariates)
...
clust_gam <- kmeans_result$cluster
...
adhd_info_lm <- fread(paste0(outpath, "/adhd_biotype_info.csv"))
clust_lm <- adhd_info_lm$biotype

# Calculate agreement
table(clust_gam, clust_lm)

if (table(clust_gam, clust_lm)[1] < 50) {
  clust_gam <- 3 - clust_gam
}

table(clust_gam, clust_lm)

adhd_demo_info <- filter(sub_demo_info, diagnosis == "adhd")
adhd_info_gam <- filter(adhd_demo_info, eventname == "baseline_year_1_arm_1")
adhd_info_gam$biotype <- paste0("biotype", clust_gam)

ari_atlas <- adjustedRandIndex(clust_gam, clust_lm)
...
for (clust_i in 1:clust_num) {
  clust_idx <- which(clust_gam == clust_i)
  adhd_deviation <- colMeans(adhd_0y_deviation_roi[clust_idx, ])
  sample_size <- table(clust_gam)[clust_i]
  ...
  plot_surface_thr(data = brain_plot_data, thr = thr, atlas = get(paste0("schaefer7_", cortex_num)), 
                   cmap = cmap, outpath = paste0(outpath_method, "/biotype", clust_i, "_deviation"))
}
```
...
adhd1_0y_idx <- which(clust_gam == 1)
adhd2_0y_idx <- which(clust_gam == 2)
...
idx1 <- which(clust_gam == 1)
idx2 <- which(clust_gam == 2)
...
sub_info_stats <- merge(sub_demo_info, adhd_info_gam[, c("src_subject_id", "biotype")], 
...
adhd_info_gam_0y_compare <- select(adhd_info_gam_0y, all_of(data_var_compare))

# Compare groups
res_0y <- compareGroups(biotype ~ ., data = as.data.table(adhd_info_gam_0y_compare))
...
```{r plot_demo_diff}
adhd_biotype1 <- filter(adhd_info_gam_0y_compare, biotype == "biotype1")
adhd_biotype2 <- filter(adhd_info_gam_0y_compare, biotype == "biotype2")
...
p <- ggplot(adhd_info_gam_0y_compare, aes(x = biotype, y = cbcl_scr_syn_attention_t, fill = biotype)) +
  geom_violin(alpha = 0.4, trim = FALSE) +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black", stroke = 1) +
...
p <- ggplot(adhd_info_gam_0y_compare, aes(x = biotype, y = cbcl_scr_syn_external_t, fill = biotype)) +
