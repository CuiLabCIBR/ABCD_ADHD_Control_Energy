---
title: "ADHD Energy Deviation Biotype - State Validation"
author:
  - name: Hang Yang
...
outpath <- file.path(root_path, "results", "all0In_2Back0Back", "schaefer400")
dir.create(outpath, recursive = TRUE, showWarnings = FALSE)

outpath_contrast <- file.path(root_path, "results", task_contrast, atlas)
dir.create(outpath_contrast, recursive = TRUE, showWarnings = FALSE)
...
clust_0Back_2Back <- kmeans_result$cluster

table(clust_0Back_2Back)
```
...
adhd_info_all0In_2Back0Back <- fread(paste0(outpath, "/adhd_biotype_info.csv"))
clust_all0In_2Back0Back <- adhd_info_all0In_2Back0Back$biotype

# Calculate agreement
table(clust_0Back_2Back, clust_all0In_2Back0Back)

if (table(clust_0Back_2Back, clust_all0In_2Back0Back)[1] < 50) {
  clust_0Back_2Back <- 3 - clust_0Back_2Back
}

ari_atlas <- adjustedRandIndex(clust_0Back_2Back, clust_all0In_2Back0Back)
print(paste("Adjusted Rand Index (Atlas Validation):", round(ari_atlas, 3)))

table(clust_0Back_2Back, clust_all0In_2Back0Back)

adhd_demo_info <- filter(sub_demo_info, diagnosis == "adhd")
adhd_info_0Back_2Back <- filter(adhd_demo_info, eventname == "baseline_year_1_arm_1")
adhd_info_0Back_2Back$biotype <- paste0("biotype", clust_0Back_2Back)
```
...
for (clust_i in 1:clust_num) {
  clust_idx <- which(clust_0Back_2Back == clust_i)
  adhd_deviation <- colMeans(adhd_0y_deviation_roi[clust_idx, ])
  sample_size <- table(clust_0Back_2Back)[clust_i]
  brain_plot_data <- data.frame(label = atlas_label, deviation = adhd_deviation[1:cortex_num])
  thr <- 0.6
  
  plot_surface_thr(data = brain_plot_data, thr = thr, atlas = get(paste0("schaefer7_", cortex_num)), cmap = cmap, outpath = paste0(outpath_contrast, "/biotype", clust_i, "_deviation"))
}
```
...
adhd1_0y_idx <- which(clust_0Back_2Back == 1)
adhd2_0y_idx <- which(clust_0Back_2Back == 2)
...
boxplot_wholebrain_diff <- function(td_deviation, adhd_deviation, biotype, outpath_contrast) {
  ...
  print(p)
  ggsave(file.path(outpath_contrast, paste0(biotype, "_wholebrain_boxplot.png")), bg = "transparent",
...
stats_tmp <- boxplot_wholebrain_diff(td1_0y_deviation_wholebrain, adhd1_0y_deviation_wholebrain, "biotype1", outpath_contrast)
...
stats_tmp <- boxplot_wholebrain_diff(td2_0y_deviation_wholebrain, adhd2_0y_deviation_wholebrain, "biotype2", outpath_contrast)
energy_wholebrain_stats <- rbind(energy_wholebrain_stats, stats_tmp)
...
write.csv(energy_wholebrain_stats, paste0(outpath_contrast, "/energy_wholebrain_stats.csv"), row.names = FALSE)
```
...
idx1 <- which(clust_0Back_2Back == 1)
idx2 <- which(clust_0Back_2Back == 2)
...
for (type in names(counts_list)) {
  ...
  print(p)
  ggsave(paste0(outpath_contrast, "/plot_adhd_deviation_count_", type, ".png"), bg = "transparent", plot = p, 
...
sub_info_stats <- merge(sub_demo_info, adhd_info_0Back_2Back[, c("src_subject_id", "biotype")], 
...
adhd_info_0Back_2Back_0y <- sub_info_stats %>% 
  filter(eventname == "baseline_year_1_arm_1", diagnosis == "adhd", usage == "test")

# Convert to factors
factor_var <- c("sex", "race", "handedness", "diagnosis")
adhd_info_0Back_2Back_0y[, (factor_var) := lapply(.SD, as.factor), .SDcols = factor_var]
...
adhd_info_0Back_2Back_0y_compare <- select(adhd_info_0Back_2Back_0y, all_of(data_var_compare))

# Compare groups
res_0y <- compareGroups(biotype ~ ., data = as.data.table(adhd_info_0Back_2Back_0y_compare))
restab_0y <- createTable(res_0y)
print(restab_0y)

# Export to Word
export2word(restab_0y, paste0(outpath_contrast, "/biotype_demo_diff_0y.docx"))
```

```{r plot_demo_diff}
adhd_biotype1 <- filter(adhd_info_0Back_2Back_0y_compare, biotype == "biotype1")
adhd_biotype2 <- filter(adhd_info_0Back_2Back_0y_compare, biotype == "biotype2")
...
p <- ggplot(adhd_info_0Back_2Back_0y_compare, aes(x = biotype, y = cbcl_scr_syn_attention_t, fill = biotype)) +
  geom_violin(alpha = 0.4, trim = FALSE) +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black", stroke = 1) +
...
print(p)
ggsave(paste0(outpath_contrast, "/plot_biotype_attention_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)

p <- ggplot(adhd_info_0Back_2Back_0y_compare, aes(x = biotype, y = cbcl_scr_syn_external_t, fill = biotype)) +
...
print(p)
ggsave(paste0(outpath_contrast, "/plot_biotype_external_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)
```
...
biotype_info <- adhd_info_0Back_2Back
...
ggsave(file.path(outpath_contrast, "adhd_longitudinal_energy_diff.png"), plot = p, width = 13, height = 11, units = "cm", dpi = 1200)
```
### Paired t-test
...
print(p)
ggsave(file.path(outpath_contrast, "corr_energy_diff_attention_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
...
print(p)
ggsave(file.path(outpath_contrast, "corr_energy_diff_external_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
