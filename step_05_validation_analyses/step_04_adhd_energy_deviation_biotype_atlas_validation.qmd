---
title: "ADHD Energy Deviation Biotype - Atlas Validation"
author:
  - name: Hang Yang
...
outpath <- file.path(root_path, "results", task_contrast, "schaefer400")
dir.create(outpath, recursive = TRUE, showWarnings = FALSE)

outpath_atlas <- file.path(root_path, "results", task_contrast, atlas)
dir.create(outpath_atlas, recursive = TRUE, showWarnings = FALSE)
...
clust_schaefer200 <- kmeans_result$cluster

table(clust_schaefer200)
```
...
adhd_info_schaefer400 <- fread(paste0(outpath, "/adhd_biotype_info.csv"))
clust_schaefer400 <- adhd_info_schaefer400$biotype

# Calculate agreement
table(clust_schaefer200, clust_schaefer400)

if (table(clust_schaefer200, clust_schaefer400)[1] < 50) {
  clust_schaefer200 <- 3 - clust_schaefer200
}

ari_atlas <- adjustedRandIndex(clust_schaefer200, clust_schaefer400)
print(paste("Adjusted Rand Index (Atlas Validation):", round(ari_atlas, 3)))

table(clust_schaefer200, clust_schaefer400)

adhd_demo_info <- filter(sub_demo_info, diagnosis == "adhd")
adhd_info_schaefer200 <- filter(adhd_demo_info, eventname == "baseline_year_1_arm_1")
adhd_info_schaefer200$biotype <- paste0("biotype", clust_schaefer200)
```
...
for (clust_i in 1:clust_num) {
  clust_idx <- which(clust_schaefer200 == clust_i)
  adhd_deviation <- colMeans(adhd_0y_deviation_roi[clust_idx, ])
  sample_size <- table(clust_schaefer200)[clust_i]
  ...
  plot_surface_thr(data = brain_plot_data, thr = thr, atlas = get(paste0("schaefer7_", cortex_num)), cmap = cmap, 
                   outpath = paste0(outpath_atlas, "/biotype", clust_i, "_deviation"))
}
```
...
adhd1_0y_idx <- which(clust_schaefer200 == 1)
adhd2_0y_idx <- which(clust_schaefer200 == 2)
...
boxplot_wholebrain_diff <- function(td_deviation, adhd_deviation, biotype, outpath_atlas) {
  ...
  print(p)
  ggsave(file.path(outpath_atlas, paste0(biotype, "_wholebrain_boxplot.png")), bg = "transparent",
...
stats_tmp <- boxplot_wholebrain_diff(td1_0y_deviation_wholebrain, adhd1_0y_deviation_wholebrain, "biotype1", outpath_atlas)
...
stats_tmp <- boxplot_wholebrain_diff(td2_0y_deviation_wholebrain, adhd2_0y_deviation_wholebrain, "biotype2", outpath_atlas)
energy_wholebrain_stats <- rbind(energy_wholebrain_stats, stats_tmp)
...
write.csv(energy_wholebrain_stats, paste0(outpath_atlas, "/energy_wholebrain_stats.csv"), row.names = FALSE)
```
...
idx1 <- which(clust_schaefer200 == 1)
idx2 <- which(clust_schaefer200 == 2)
...
for (type in names(counts_list)) {
  ...
  print(p)
  ggsave(paste0(outpath_atlas, "/plot_adhd_deviation_count_", type, ".png"), bg = "transparent", plot = p, 
...
sub_info_stats <- merge(sub_demo_info, adhd_info_schaefer200[, c("src_subject_id", "biotype")], 
...
adhd_info_schaefer200_0y <- sub_info_stats %>% 
  filter(eventname == "baseline_year_1_arm_1", diagnosis == "adhd", usage == "test")

# Convert to factors
factor_var <- c("sex", "race", "handedness", "diagnosis")
adhd_info_schaefer200_0y[, (factor_var) := lapply(.SD, as.factor), .SDcols = factor_var]
...
adhd_info_schaefer200_0y_compare <- select(adhd_info_schaefer200_0y, all_of(data_var_compare))

# Compare groups
res_0y <- compareGroups(biotype ~ ., data = as.data.table(adhd_info_schaefer200_0y_compare))
restab_0y <- createTable(res_0y)
print(restab_0y)

# Export to Word
export2word(restab_0y, paste0(outpath_atlas, "/biotype_demo_diff_0y.docx"))
```

```{r plot_demo_diff}
adhd_biotype1 <- filter(adhd_info_schaefer200_0y_compare, biotype == "biotype1")
adhd_biotype2 <- filter(adhd_info_schaefer200_0y_compare, biotype == "biotype2")
...
p <- ggplot(adhd_info_schaefer200_0y_compare, aes(x = biotype, y = cbcl_scr_syn_attention_t, fill = biotype)) +
  geom_violin(alpha = 0.4, trim = FALSE) +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black", stroke = 1) +
...
print(p)
ggsave(paste0(outpath_atlas, "/plot_biotype_attention_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)

p <- ggplot(adhd_info_schaefer200_0y_compare, aes(x = biotype, y = cbcl_scr_syn_external_t, fill = biotype)) +
...
print(p)
ggsave(paste0(outpath_atlas, "/plot_biotype_external_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)
```
...
biotype_info <- adhd_info_schaefer200
...
ggsave(file.path(outpath_atlas, "adhd_longitudinal_energy_diff.png"), plot = p, width = 13, height = 11, units = "cm", dpi = 1200)
```
### Paired t-test
...
print(p)
ggsave(file.path(outpath_atlas, "corr_energy_diff_attention_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
...
print(p)
ggsave(file.path(outpath_atlas, "corr_energy_diff_external_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
