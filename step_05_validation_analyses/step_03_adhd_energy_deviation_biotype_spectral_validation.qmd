---
title: "ADHD Energy Deviation Biotype - Spectral Clustering Validation"
author:
  - name: Hang Yang
...
source("F:/Cui_Lab/Projects/ABCD_ADHD_Control_Energy/functions/estimate_cluster_number.R")
...
outpath_method <- file.path(root_path, "results", task_contrast, atlas, "spectral")
dir.create(outpath_method, recursive = TRUE, showWarnings = FALSE)
...
# Calculate similarity matrix and perform spectral clustering
simil_matrix <- proxy::simil(adhd_0y_deviation_roi, method = "correlation")
...
k_vals  <- 2:12
estimation_result <- estimate_cluster_number(affinity_matrix, k_vals)
clust_num <- estimation_result$`Eigen-gap best`
clust_spectral <- spectralClustering(affinity_matrix, clust_num)

table(clust_spectral)
```

```{r plot_spectral_curve}
df_metrics <- data.frame(
  k = k_vals,
  eigengap = estimation_result$eigengap,
  rotation_cost = estimation_result$rotation_cost
)

p <- ggplot(df_metrics, aes(x = k, y = eigengap)) +
  ...
  theme(legend.position = "none", axis.text = element_text(size = 16, color = "black"),
        ...
        aspect.ratio = 0.8)
print(p)
ggsave(paste0(outpath_method, "/plot_eigengap.png"), bg = "transparent", 
       plot = p, width = 11, height = 9, units = "cm", dpi = 1200)


p <- ggplot(df_metrics, aes(x = k, y = rotation_cost)) +
  geom_line(linewidth = 1, color = "#6baed6") +
  geom_point(size = 2, color = "#5A8DB0") +
  labs(
    x = "Number of clusters",
  ...
  theme(legend.position = "none", axis.text = element_text(size = 16, color = "black"),
        ...
        aspect.ratio = 0.8)
print(p)
ggsave(paste0(outpath_method, "/plot_rotation_cost.png"), bg = "transparent", 
       plot = p, width = 11, height = 9, units = "cm", dpi = 1200)
...
adhd_info_kmeans <- fread(paste0(outpath, "/adhd_biotype_info.csv"))
clust_kmeans <- adhd_info_kmeans$biotype

# Calculate agreement
table(clust_spectral, clust_kmeans)

if (table(clust_spectral, clust_kmeans)[1] < 50) {
  clust_spectral <- 3 - clust_spectral
}

table(clust_spectral, clust_kmeans)

adhd_demo_info <- filter(sub_demo_info, diagnosis == "adhd")
adhd_info_spectral <- filter(adhd_demo_info, eventname == "baseline_year_1_arm_1")
adhd_info_spectral$biotype <- paste0("biotype", clust_spectral)

ari_atlas <- adjustedRandIndex(clust_spectral, clust_kmeans)
...
for (clust_i in 1:clust_num) {
  clust_idx <- which(clust_spectral == clust_i)
  adhd_deviation <- colMeans(adhd_0y_deviation_roi[clust_idx, ])
  sample_size <- table(clust_spectral)[clust_i]
  ...
  plot_surface_thr(data = brain_plot_data, thr = thr, atlas = get(paste0("schaefer7_", cortex_num)), cmap = cmap,
                   outpath = paste0(outpath_method, "/biotype", clust_i, "_deviation"))
}
```
...
adhd1_0y_idx <- which(clust_spectral == 1)
adhd2_0y_idx <- which(clust_spectral == 2)
...
boxplot_wholebrain_diff <- function(td_deviation, adhd_deviation, biotype, outpath_method) {
  ...
  print(p)
  ggsave(file.path(outpath_method, paste0(biotype, "_wholebrain_boxplot.png")), bg = "transparent",
...
stats_tmp <- boxplot_wholebrain_diff(td1_0y_deviation_wholebrain, adhd1_0y_deviation_wholebrain, "biotype1", outpath_method)
...
stats_tmp <- boxplot_wholebrain_diff(td2_0y_deviation_wholebrain, adhd2_0y_deviation_wholebrain, "biotype2", outpath_method)
energy_wholebrain_stats <- rbind(energy_wholebrain_stats, stats_tmp)
...
write.csv(energy_wholebrain_stats, paste0(outpath_method, "/energy_wholebrain_stats.csv"), row.names = FALSE)
```
...
idx1 <- which(clust_spectral == 1)
idx2 <- which(clust_spectral == 2)
...
for (type in names(counts_list)) {
  ...
  print(p)
  ggsave(paste0(outpath_method, "/plot_adhd_deviation_count_", type, ".png"), bg = "transparent", plot = p, 
...
sub_info_stats <- merge(sub_demo_info, adhd_info_spectral[, c("src_subject_id", "biotype")], 
...
adhd_info_spectral_0y <- sub_info_stats %>% 
  filter(eventname == "baseline_year_1_arm_1", diagnosis == "adhd", usage == "test")

# Convert to factors
factor_var <- c("sex", "race", "handedness", "diagnosis")
adhd_info_spectral_0y[, (factor_var) := lapply(.SD, as.factor), .SDcols = factor_var]
...
adhd_info_spectral_0y_compare <- select(adhd_info_spectral_0y, all_of(data_var_compare))

# Compare groups
res_0y <- compareGroups(biotype ~ ., data = as.data.table(adhd_info_spectral_0y_compare))
restab_0y <- createTable(res_0y)
print(restab_0y)

# Export to Word
export2word(restab_0y, paste0(outpath_method, "/biotype_demo_diff_0y.docx"))
```

```{r plot_demo_diff}
adhd_biotype1 <- filter(adhd_info_spectral_0y_compare, biotype == "biotype1")
adhd_biotype2 <- filter(adhd_info_spectral_0y_compare, biotype == "biotype2")
...
p <- ggplot(adhd_info_spectral_0y_compare, aes(x = biotype, y = cbcl_scr_syn_attention_t, fill = biotype)) +
  geom_violin(alpha = 0.4, trim = FALSE) +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black", stroke = 1) +
...
print(p)
ggsave(paste0(outpath_method, "/plot_biotype_attention_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)

p <- ggplot(adhd_info_spectral_0y_compare, aes(x = biotype, y = cbcl_scr_syn_external_t, fill = biotype)) +
...
print(p)
ggsave(paste0(outpath_method, "/plot_biotype_external_diff.png"), bg = "transparent", 
       plot = p, width = 10, height = 11, units = "cm", dpi = 1200)
```
...
biotype_info <- adhd_info_spectral
...
ggsave(file.path(outpath_method, "adhd_longitudinal_energy_diff.png"), plot = p, width = 13, height = 11, units = "cm", dpi = 1200)
```
...
print(p)
ggsave(file.path(outpath_method, "corr_energy_diff_attention_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
...
print(p)
ggsave(file.path(outpath_method, "corr_energy_diff_external_diff.png"), plot = p, width = 12, height = 11, units = "cm", dpi = 1200)
```
