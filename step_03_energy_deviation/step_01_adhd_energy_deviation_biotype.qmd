...
subcortex_path <- file.path(root_path, "results/plots", task_contrast, "subcortex")
...
fwrite(data.frame(adhd_pos_deviation_pct[401:452]), paste0(subcortex_path, "/adhd_pos_deviation_pct.csv"), row.names = FALSE)
fwrite(data.frame(adhd_neg_deviation_pct[401:452]), paste0(subcortex_path, "/adhd_neg_deviation_pct.csv"), row.names = FALSE)
...
adhd1_0y_deviation_roi <- colMeans(adhd_0y_deviation_roi[which(clust == 1), ])
adhd2_0y_deviation_roi <- colMeans(adhd_0y_deviation_roi[which(clust == 2), ])
fwrite(data.frame(adhd1_0y_deviation_roi[401:452]), paste0(subcortex_path, "/adhd1_0y_deviation_roi.csv"), row.names = FALSE)
fwrite(data.frame(adhd2_0y_deviation_roi[401:452]), paste0(subcortex_path, "/adhd2_0y_deviation_roi.csv"), row.names = FALSE)
```
...
counts_list <- list(
  all = list(biotype1 = adhd_counts[idx1], biotype2 = adhd_counts[idx2], ylabel = "Total deviation count"),
  pos = list(biotype1 = adhd_pos_counts[idx1], biotype2 = adhd_pos_counts[idx2], ylabel = "Positive deviation count"),
  neg = list(biotype1 = adhd_neg_counts[idx1], biotype2 = adhd_neg_counts[idx2], ylabel = "Negative deviation count")
)
...
```{r}
# Compare positive and negative extreme deviation counts within each biotype
idx1 <- which(clust == 1)
idx2 <- which(clust == 2)

counts_list <- list(
  "ADHD-1" = list(pos = adhd_pos_counts[idx1], neg = adhd_neg_counts[idx1]),
  "ADHD-2" = list(pos = adhd_pos_counts[idx2], neg = adhd_neg_counts[idx2])
)

stats_results <- data.frame()

for (bt in names(counts_list)) {
  model_data <- data.frame(
    pos = counts_list[[bt]]$pos,
    neg = counts_list[[bt]]$neg
  ) %>% drop_na()
  
  pos_counts <- model_data$pos
  neg_counts <- model_data$neg
  
  t_test_result <- t.test(pos_counts, neg_counts, paired = TRUE)
  wilcox_result <- wilcox.test(pos_counts, neg_counts, paired = TRUE, exact = FALSE)
  
  summary_vals <- data.frame(
    Biotype = bt,
    N = nrow(model_data),
    Pos_Mean = mean(pos_counts),
    Pos_SD = sd(pos_counts),
    Neg_Mean = mean(neg_counts),
    Neg_SD = sd(neg_counts),
    Mean_Diff = mean(pos_counts - neg_counts)
  )
  
  stats_results <- bind_rows(
    stats_results,
    cbind(summary_vals, Test = "paired t-test", Statistic = unname(t_test_result$statistic), PValue = t_test_result$p.value),
    cbind(summary_vals, Test = "paired wilcox", Statistic = unname(wilcox_result$statistic), PValue = wilcox_result$p.value)
  )
}
...
nback_item <- c("nihtbx_list_uncorrected", "tfmri_nb_all_beh_c0b_rate", "tfmri_nb_all_beh_c2b_rate", "tfmri_nb_all_beh_ctotal_rate",
                "tfmri_nb_all_beh_c0b_mrt", "tfmri_nb_all_beh_c2b_mrt", "tfmri_nb_all_beh_ctotal_mrt")
data_var <- c(demo_var, dx_var, mri_var, cbcl_var, nback_item)
...
# Convert to factors
factor_var <- c("sex", "race", "handedness", "diagnosis", "biotype")
adhd_info_0y[, (factor_var) := lapply(.SD, as.factor), .SDcols = factor_var]
adhd_info_0y[, biotype := factor(biotype, levels = c("biotype1", "biotype2"))]
...
lm_results <- rbindlist(lapply(nback_item, function(var) {
  
  model_data <- adhd_info_0y[
    complete.cases(adhd_info_0y[, .SD, .SDcols = c(var, "biotype", "age", "sex")])
  ]
  
  fit <- lm(reformulate(c("biotype", "age", "sex"), response = var), data = model_data)
  coef_table <- summary(fit)$coefficients
  
  bio_coef <- coef_table["biotypebiotype2", ]
  
  data.table(
    variable = var,
    mean_biotype1 = round(mean(model_data[biotype == "biotype1"][[var]], na.rm = TRUE), 2),
    sd_biotype1 = round(sd(model_data[biotype == "biotype1"][[var]], na.rm = TRUE), 2),
    mean_biotype2 = round(mean(model_data[biotype == "biotype2"][[var]], na.rm = TRUE), 2),
    sd_biotype2 = round(sd(model_data[biotype == "biotype2"][[var]], na.rm = TRUE), 2),
    t = bio_coef["t value"],
    p = bio_coef["Pr(>|t|)"]
  )
}))

lm_results[, p_fdr := p.adjust(p, method = "fdr")]
lm_results[, p_sig := as.integer(p < 0.05)]
lm_results[, p_fdr_sig := as.integer(p_fdr < 0.05)]
