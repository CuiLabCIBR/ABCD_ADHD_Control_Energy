...
CSV_FILE="/ibmgpfs/cuizaixu_lab/yanghang/projects/abcd_adhd_control_energy/data/sub_info/sub_demo_info.csv"
...
do
 OUT_PATH="$OUT_DIR/EnergyCal_${subj}.out"
 LOG_PATH="$LOG_DIR/EnergyCal_${subj}.log"
 sbatch -o "$OUT_PATH" -e "$LOG_PATH" "$BASE_DIR/EnergyCal.sh" "$subj"
done
