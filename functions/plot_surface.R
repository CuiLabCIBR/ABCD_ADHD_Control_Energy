plot_surface <- function(data, outpath, color_type, atlas, cmap) {
  ...
  #cmap <- rev(c("#DD7969" , "#F4A582" , "#FDDBC7", "#F7F7F7", "#D1E5F0", "#92C5DE", "#549CC9"))
  ...
  if (color_type == 2) {
    maxup <- max(data$value)
    minup <- min(data$value)
  ...
  if (color_type == 3) {
    ...
    values1 <- scales::rescale(c(1:100), to = c(zero, maxup))
    values2 <- scales::rescale(c(1:100), to = c(minup, -zero))
    values <- c(values2, values1)
    values <- scales::rescale(values, to = c(0, 1))
  }
  
  if (color_type == 4) {
    max_all <- max(abs(data$value))
    zero <- 1e-15
    
    values1 <- scales::rescale(c(1:100), to = c(zero, max_all))
    values2 <- scales::rescale(c(1:100), to = c(-max_all, -zero))
    values <- c(values2, values1)
    values <- scales::rescale(values, to = c(0, 1))
  }
  
  p <- ggplot(data = data)+
    geom_brain(aes(fill = value, colour = value), atlas = atlas, position = position_brain(hemi~side)) +
  ...
  # Create a dummy plot with only the color scale
  dummy_plot <- ggplot(data = data, aes(x = 1, y = value, fill = value)) +
    geom_tile() + scale_fill_gradientn(colours = cmap, values = values) +
    theme_void() + theme(legend.position = "right")

  ggsave(paste0(outpath, '_colorbar.png'), bg = 'transparent', plot = dummy_plot, width = 2, height = 6, units = "cm", dpi = 1200)
